...
CINormMuR6 <- R6Class(
  ...
  public = list(
    ...
    createArraysToSave = function() {
      tQuantile  = -qt(p = self$getAlpha() / 2,
                       df = self$getSampleSize() - 1)
      ...
      isMuCovered = (self$getMu() < confintUpper &
                       self$getMu() > confintLower)
    ...
    createImageInfo = function() {
      ...
      self$imageInfo[1] <- "Confidence intervals for a mean of normal distribution"
